...
export class ArchivoService {
  ...
  get(archivoid = null, fuente: boolean = false): Observable<Model[]> {
    ...
    // console.log(archivoid+' - '+fuente);
    return this._http.get(_url, _options)
      .map((response: Response) => {
        // console.log(this._autenticacionService.extractData(response).length);
        // if(this._autenticacionService.extractData(response).length<2)
        // console.log(this._autenticacionService.extractData(response));
        const archivos: Model[] = this._autenticacionService.extractData(response);
        if (archivoid !== '' && archivoid !== null) {
          // console.log(archivos);
          return archivos;
        } else {
          if (!fuente) {
            const archivosPDF: Model[] = [];
            archivos.forEach(function (archivo: Model) {
              if (archivo.vinculadoa === undefined || archivo.vinculadoa === null || archivo.vinculadoa === '') {
                archivosPDF.push(archivo);
              }
            });
            return archivosPDF;
          } else {
            const archivosFuentes: Model[] = [];
            archivos.forEach(function (archivo: Model) {
              // console.log(archivo.archivoid+' - '+archivo.vinculadoa);
              if (archivo.vinculadoa !== null && archivo.vinculadoa !== '') {
                // console.log('Ebert');
                // console.log(archivo);
                archivosFuentes.push(archivo);
              }
            });
  ...
  getObsoletos(archivoid = null): Observable<Model[]> {
    const _headers = new Headers({ 'Authorization': 'Bearer ' + this._autenticacionService.token });
    const _options = new RequestOptions({ headers: _headers });
    let _url = this.URL + '/obsoletos/';
    if (archivoid !== null) { _url += archivoid; }
    // console.log(archivoid+' - '+fuente);
    return this._http.get(_url, _options)
      .map((response: Response) => {
  ...
  listadoMaestro(): Observable<any> {
    const _headers = new Headers({ 'Authorization': 'Bearer ' + this._autenticacionService.token });
    const _options = new RequestOptions({ headers: _headers });
    const _url = app.apiurl + 'archivos.php/listadomaestro';
    // console.log(_url);
